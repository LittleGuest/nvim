...
1. clone本仓库到 `~/config/nvim` 目录下

```shell
# 直接clone到~/.config/nvim/位置
git clone --depth 1 https://github.com/LittleGuest/nvim.git ~/.config/nvim/
```

2. 执行 `nvim +PackerSync` 命令， 根据网络条件的不同，可能需要多次执行本命令，直到所有插件下载完成

3. 通过`LspInstallInfo`命令安装对应lsp, 接着选中对应语言的server，如`rust`为`rust_analyzer`， 按`i`执行安装, `X`执行卸载

4. 通过`DIInstall xxx`命令安装对应语言的 `debugger`, 除此外，需要在 `~/nvim/lua/user/dap/`目录下新建对应语言的配置文件（已配置好c/cpp,go,python), 同时更新 `dap-config.lua` 文件下的 `config_debuggers()` 函数。
